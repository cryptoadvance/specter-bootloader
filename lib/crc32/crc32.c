/**
 * @file       crc32.c
 * @brief      "Fast CRC32" implementation
 * @author     Stephan Brumme (original implementation)
 * @author     Mike Tolkachev <contact@miketolkachev.dev> (refactoring)
 * @copyright  Copyright (c) 2016-2019 Stephan Brumme. All rights reserved.
 *
 * License for the code, zlib, is included in the end of this file.
 *
 * Modifications to the original Crc32.cpp:
 *   - Dropped support of 16-byte look-up table
 *   - Removed all algorithms not called from crc32_fast()
 *   - Removed C++ namespace
 *   - Not exposed functions and constants made static
 *   - Removed definition of prefetch macros (not used in remaining functions)
 *   - MaxSlice constant changed to MAX_SLICE define
 *   - Added compile optimisation attribute to core functions
 *
 * GitHub repository of original implementation by Stephan Brumme:
...
static uint32_t ATTR_OPT crc32_halfbyte(const void* data, size_t length,
...
static uint32_t ATTR_OPT crc32_1byte(const void* data, size_t length,
                                     uint32_t previousCrc32)
{
  uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
  const uint8_t* current = (const uint8_t*) data;
...
static uint32_t ATTR_OPT crc32_4bytes(const void* data, size_t length,
                                      uint32_t previousCrc32)
{
  uint32_t  crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
...
static uint32_t ATTR_OPT crc32_8bytes(const void* data, size_t length,
                                      uint32_t previousCrc32)
{
  uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
  const uint32_t* current = (const uint32_t*) data;
...
static uint32_t ATTR_OPT crc32_4x8bytes(const void* data, size_t length,
                                        uint32_t previousCrc32)
{
  uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
  const uint32_t* current = (const uint32_t*) data;
...
static uint32_t ATTR_OPT crc32_16bytes(const void* data, size_t length,
                                       uint32_t previousCrc32)
{
  uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
  const uint32_t* current = (const uint32_t*) data;
