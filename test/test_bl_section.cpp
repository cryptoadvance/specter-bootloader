...
public:
  ...
  inline operator bl_file_t() const {
    return (bl_file_t)fd;
  }
...
TEST_CASE("Validate header") {
  SECTION("valid") {
    SECTION("reference header") {
      REQUIRE( bl_validate_header(&ref_header) );
    }
  ...
  SECTION("invalid") {
    SECTION("NULL pointer") {
      REQUIRE_FALSE( bl_validate_header(NULL) );
    }
...
TEST_CASE("Validate payload") {
  SECTION("valid, reference payload") {
    REQUIRE(
      bl_validate_payload(&ref_header, ref_payload, sizeof(ref_payload))
    );
  }
  ...
  SECTION("invalid, broken CRC") {
    ...
    hdr.pl_crc ^= 1U;
    REQUIRE_FALSE(
      bl_validate_payload(correct_crc(&hdr), ref_payload, sizeof(ref_payload))
    );
  }
}
...
TEST_CASE("Check if payload section") {
  SECTION("reference header") {
    REQUIRE( bl_section_is_payload(&ref_header) );
  }
